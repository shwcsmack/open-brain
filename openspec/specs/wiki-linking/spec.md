...
The system SHALL display an autocomplete dropdown of up to 10 matching note titles when the user types `[[` followed by at least one character.

#### Scenario: Autocomplete triggered
- **WHEN** the user types `[[Bio` in the editor
- **THEN** a dropdown appears showing notes whose titles contain "Bio"

#### Scenario: Autocomplete selection
- **WHEN** the user selects "Biology" from the autocomplete dropdown
- **THEN** a resolved `wikilink` node for "Biology" is inserted and the dropdown closes
