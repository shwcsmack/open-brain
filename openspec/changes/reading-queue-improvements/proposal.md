...
- To: One `ReadingItem` per Wikipedia article; all sections concatenated. Section picker removed from the Add page and the in-session link handler.
- Reason: Users want to clean up articles themselves while reading (using delete-passage) rather than pre-selecting sections.
- Impact: Breaking change to `addWikipediaToQueue` signature and the Add page UX.
